...
def upgrade() -> None:
    ...
    # Pruning reads by expiry, and nothing else does.
    ...
    op.add_column('tokens', sa.Column('owner_notified_at', sa.DateTime(timezone=True), nullable=True))


def downgrade() -> None:
    """Downgrade schema."""
    op.drop_column('tokens', 'owner_notified_at')
